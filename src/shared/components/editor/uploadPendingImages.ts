...
//
// 반환: 업로드에 성공한 { localId, imageId } 목록.
// - localId: 호출부의 cleanup(IndexedDB 정리) 대상
// - imageId: 업로드 전 localId 기준으로 잡아둔 선택(대표 이미지 등)을 확정 id로 옮기기 위한 매핑
...
export async function uploadPendingImages(
  ...
  const uploadedRefs: UploadedImageRef[] = [];
  ...
  for (let i = 0; i < pendingLocalIds.length; i++) {
    ...
    if (!record) {
      failure = {
        index: i + 1,
        cause: new Error(`임시 저장된 이미지를 찾을 수 없습니다 (${localId})`),
      };
      break;
    }
    try {
      ...
      uploadedRefs.push({ localId, imageId: uploaded.imageId });
    } catch (cause) {
  ...
  if (failure) {
    ...
    const code =
      failure.cause instanceof ImageUploadError
        ? failure.cause.code
        : undefined;
    const error = new Error(
      `이미지 ${failure.index}번째 업로드 실패, 다시 저장을 시도해주세요`,
    );
    (
      error as Error & { cause?: unknown; failedAt?: number; code?: string }
    ).cause = failure.cause;
