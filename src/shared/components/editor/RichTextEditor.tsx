...
import { extractEditorImages, type EditorImage } from './extractEditorImages';
...
export interface EditorPayload {
  ...
  // onImagesChange로 받은 EditorImage.key → 업로드 확정 imageId.
  // 저장 전 선택(대표 이미지 등)을 서버 id로 되돌리는 용도.
  imageIdByKey: Record<string, number>;
}
...
interface RichTextEditorProps {
  ...
  placeholder?: string;
  // 본문 이미지 목록이 바뀔 때마다 호출(대표 이미지 선택 UI 등 페이지 쪽 용도).
  // 미전달 시 추출 자체를 건너뛴다 — 공지/FAQ 화면은 영향 없음.
...
>(function RichTextEditor(
  ...
  const { insertFiles, revokeObjectUrls, cleanup } =
    useDeferredImageUpload(ownerType);
  const fileInputRef = useRef<HTMLInputElement>(null);
  const editorRef = useRef<Editor | null>(null);

  // 콜백은 ref로 들고 있는다 — useEditor 옵션에 직접 넣으면 렌더마다 에디터 옵션이 흔들린다.
  const onImagesChangeRef = useRef(onImagesChange);
  onImagesChangeRef.current = onImagesChange;

  // 이미지 구성이 실제로 바뀐 경우에만 통지한다(타이핑마다 상위 리렌더 방지).
  const lastImageKeysRef = useRef<string | null>(null);
  const notifyImagesChange = useCallback((current: Editor) => {
    if (!onImagesChangeRef.current) return;
    const images = extractEditorImages(current);
    const keys = images.map((image) => image.key).join(',');
    if (keys === lastImageKeysRef.current) return;
    lastImageKeysRef.current = keys;
  ...
  const editor = useEditor({
    ...
    content: initialHtml,
    onCreate: ({ editor: created }) => notifyImagesChange(created),
    onUpdate: ({ editor: updated }) => notifyImagesChange(updated),
    editorProps: {
      attributes: { class: 'focus:outline-none min-h-[280px]' },
      handlePaste: (_view, event) =>
        handleFilesFromDataTransfer(event.clipboardData),
      handleDrop: (_view, event) =>
        handleFilesFromDataTransfer((event as DragEvent).dataTransfer),
    },
  });
  ...
  useImperativeHandle(
    ...
    () => ({
      getPayload: async () => {
        ...
        // 업로드 전 key 목록을 잡아둔다 — 업로드 후에는 localId가 지워져 복원할 수 없다.
        const imagesBeforeUpload = extractEditorImages(editor);
        let uploadedRefs;
        try {
          uploadedRefs = await uploadPendingImages(editor, ownerType);
        } catch (err) {
        ...
        const imageIdByLocalId = new Map(
          uploadedRefs.map((uploaded) => [uploaded.localId, uploaded.imageId]),
        );
        const imageIdByKey: Record<string, number> = {};
        for (const image of imagesBeforeUpload) {
          const imageId =
            image.imageId ??
            (image.localId != null
              ? imageIdByLocalId.get(image.localId)
              : undefined);
          if (imageId != null) imageIdByKey[image.key] = imageId;
        }

        return { html, imageIds: extractImageIds(editor), imageIdByKey };
      },
      cleanup: async () => {
        await cleanup();
      },
