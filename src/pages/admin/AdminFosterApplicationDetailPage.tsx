...
export function AdminFosterApplicationDetailPage() {
  ...
  if (!isApplicationStatus) {
    return (
      <Navigate
        to={`/admin/active-fosters/${data.fosterId}`}
        replace
      />
    );
  }

  const canDelete = !data.deletedAt && data.status === 'REJECTED';

  const editButtonLabel =
    data.status === 'PENDING' ? '신청 처리' : '수정';
    ...
    <div className="p-6">
      ...
      <div className="mb-6 grid grid-cols-2 gap-4 rounded-md border p-5 lg:grid-cols-4">
        <div className="col-span-2">
          <span className="text-sm text-muted-foreground">신청 동물</span>
        ...
        <DetailItem label="이메일" value={data.userEmail} />
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <section className="rounded-md border p-5">
          ...
          <div className="mt-4 border-t border-border pt-4">
            <span className="text-sm text-muted-foreground">하고 싶은 말</span>
            <div className="mt-2">
              <ExpandableText
                value={data.message}
      ...
      {!data.deletedAt && (
        <div className="mt-6 flex flex-wrap justify-end gap-3 border-t border-border pt-5">
          <Button asChild className="min-w-32">
            <Link to={`/admin/fosters/${data.fosterId}/edit`}>
              {editButtonLabel}
            </Link>
          </Button>
